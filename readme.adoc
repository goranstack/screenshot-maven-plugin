// Badges
image:https://github.com/goranstack/screenshot-maven-plugin/actions/workflows/build.yml/badge.svg["Build Status", link="https://github.com/goranstack/screenshot-maven-plugin/actions/workflows/build.yml"]
...
* Java 8 or later
* Maven 3.3.1 or later (required for `${maven.multiModuleProjectDirectory}`, see <<configure-your-pom-files.adoc#,Configure your pom files>>)

== The plugin goals

The plugin has three goals for various purposes:

* Create screenshots of UI components to include in the JavaDoc of your classes. The `targetClass` of a `@Screenshot`
method may reside in a *different Maven module* — the plugin resolves source directories across all modules in the
reactor and falls back to a filesystem sibling scan when running outside a multi-module build.
...
* *Cross-module `targetClass` support*: `@Screenshot` methods whose `targetClass` belongs to a different Maven module
  now work correctly. The plugin searches all reactor project source directories and falls back to a filesystem sibling
  scan (walking up to the modules parent and scanning all subdirectories for `src/main/java`) when reactor projects
  are unavailable.

* *Robustness improvements*:
** `Error` subclasses (e.g. `NoSuchMethodError`) caught in `callScreenshotMethod` so that one bad method no longer
   causes a `BUILD FAILURE` for the entire module.
** Zero-size component guard in `ripSwingComponent` prevents empty-component exceptions.
** Rendering exceptions inside `handleFoundMethod` are caught per-component so remaining screenshots are still generated.
** Full stack traces now printed for `NullPointerException` and similar errors without a message, making failures
   easier to diagnose.

* *`SampleUtil` fixes*:
** Synthetic bridge methods and synthetic fields are skipped to avoid `IllegalAccessException`.
** `setAccessible(true)` called before reflective invocation so private/protected sample methods and fields work.
